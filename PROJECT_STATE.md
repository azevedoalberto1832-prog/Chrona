...
Palazzo é o tenant real de referência e Nayara Lash Designer é o tenant de validação multi-segmento segundo a documentação do projeto. Em produção, `chronasystem.com.br` sem parâmetros é exclusivo da plataforma Chrona e `palazzo.chronasystem.com.br` é a URL canônica da Palazzo. Enquanto o domínio curinga da Vercel não está ativo, tenants sem hostname publicado usam o fallback funcional `chronasystem.com.br/?tenant=<slug>`.
...
| Fontes selecionáveis por cliente | IMPLEMENTADO | Doze pares curados são selecionáveis em uma grade visual e aplicados ao site e ao login do tenant; não há CSS arbitrário nem upload de fontes externas. |
...
| Subdomínio automático `cliente.chronasystem.com.br` | BLOQUEADO | O frontend reconhece hostnames genéricos, mas a Vercel ainda não possui `*.chronasystem.com.br`; tenants sem hostname ativo usam temporariamente `chronasystem.com.br/?tenant=<slug>`. |
