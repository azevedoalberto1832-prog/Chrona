(function () {
  ...
  function renderSection(id, ctx) {
    ...
    if(id==="gallery") { const photos=(c.gallery||[]).map((item)=>safeUrl(typeof item==="string"?item:item?.url)).filter(Boolean);if(!photos.length)return "";return `<section class="site-section site-gallery"><div class="site-container site-gallery-layout"><div class="site-gallery-copy"><span class="site-kicker">Feito por aqui</span><h2>Trabalhos recentes</h2><p>Clientes recentes, serviços disponíveis e resultados já realizados pela equipe.</p><div class="site-gallery-tags"><span>Clientes recentes</span><span>Serviços disponíveis</span><span>Trabalhos concluídos</span></div></div><div class="site-gallery-stage" data-site-rotator>${photos.map((url,index)=>`<img class="${index===0?"active":""}" src="${url}" alt="Trabalho recente ${index+1}" loading="lazy">`).join("")}<span class="site-gallery-count">01 / ${String(photos.length).padStart(2,"0")}</span></div></div><div class="site-container site-gallery-carousel" aria-label="Galeria de trabalhos recentes">${photos.map((url,index)=>`<figure><img src="${url}" alt="Trabalho realizado ${index+1}" loading="lazy"></figure>`).join("")}</div></section>`; }
