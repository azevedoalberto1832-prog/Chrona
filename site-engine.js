(function () {
  ...
  const variants = {
    hero:["centered","split","full-image","minimal","luxury"],
  ...
  const variantNames = { centered:"Centered",split:"Split","full-image":"Full Image",minimal:"Minimal",luxury:"Luxury",solid:"Solid",outline:"Outline",pill:"Pill",classic:"Classic",border:"Border",elevated:"Elevated",grid:"Grid",list:"Lista",cards:"Cards",premium:"Premium Cards","image-cards":"Image Cards",horizontal:"Horizontal" };
  const defaultSections = ["header","hero","services","about","differentials","professionals","gallery","testimonials","location","hours","final-cta","footer"].map((id) => ({ id, visible:!["gallery","testimonials"].includes(id) }));
  ...
  const defaultContent = {
    hero:{ eyebrow:"Agendamento online",title:"",subtitle:"",ctaLabel:"Agendar horário",imageUrl:"" },
  ...
  const mergeContent = (content={}) => ({ ...clone(defaultContent),...content,hero:{...defaultContent.hero,...content.hero},about:{...defaultContent.about,...content.about},finalCta:{...defaultContent.finalCta,...content.finalCta} });
  ...
  function renderSection(id, ctx) {
    ...
    const logo=shop.logo?`<img class="site-logo" src="${safeUrl(shop.logo)}" alt="Logo ${escape(shop.name)}">`:`<span class="site-logo site-monogram">${escape(String(shop.name||"C").slice(0,1))}</span>`;
    ...
    if(id==="hero") { const image=safeUrl(c.hero.imageUrl||shop.logo); return `<section class="site-hero site-hero-${v.hero}"><div class="site-container site-hero-inner"><div class="site-hero-copy"><span class="site-kicker">${escape(c.hero.eyebrow)}</span><h1>${escape(c.hero.title||shop.name)}</h1><p>${escape(c.hero.subtitle||shop.description)}</p><div class="site-actions">${book(c.hero.ctaLabel)}${social}<a class="site-link-button" target="_blank" rel="noopener" href="${whatsapp}">Falar no WhatsApp</a></div></div>${image?`<figure class="site-hero-media"><img src="${image}" alt="${escape(shop.name)}"></figure>`:""}</div></section>`; }
    if(id==="services") { const cards=services.map((service)=>`<article class="site-card site-card-${v.card}"><div class="site-card-meta"><span>${escape(service.duration)} MIN</span><b>${money(service.price)}</b></div><h3>${escape(service.name)}</h3><p>${escape(service.desc||"Atendimento personalizado.")}</p>${book("Agendar este serviço",`data-service="${escape(service.id)}"`)}</article>`).join(""); return `<section class="site-section site-services site-services-${v.services}" id="servicos"><div class="site-container"><div class="site-heading"><span class="site-kicker">Serviços</span><h2>Escolha o seu atendimento</h2><p>Valores e disponibilidade atualizados pela equipe.</p></div><div class="site-service-list">${cards||'<div class="site-empty">Serviços em preparação.</div>'}</div></div></section>`; }
    ...
    if(id==="gallery") return `<section class="site-section site-gallery"><div class="site-container"><div class="site-heading"><span class="site-kicker">Galeria</span><h2>Nosso espaço e trabalho</h2></div><div class="site-gallery-grid">${(c.gallery||[]).map((url,index)=>safeUrl(url)?`<img src="${safeUrl(url)}" alt="Galeria ${index+1}" loading="lazy">`:"").join("")}</div></div></section>`;
  ...
  window.ChronaSite={palettes,fonts,templates,variants,variantNames,sectionNames,defaultSections,defaultContent,preset,normalize,render,search,clone};
})();
